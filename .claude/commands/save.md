...
**Step-by-step process to construct complete enriched_rems.json**:

1. **Read enriched Rems from Step 5**:
   - If Step 5 saved to `enriched_rems.json`, read the `rems` array
   - Extract: Rems list with `typed_relations` already added
   - Note: Step 5 output has incomplete session_metadata (only domain/isced_path)

2. **Construct complete session metadata** (main agent knows these values from conversation context):
   - `id`: Generate from conversation topic (e.g., "qing-cultural-policies-2025-11-24")
   - `title`: Summarize conversation focus in one sentence
   - `summary`: 2-3 sentences describing what user learned
   - `archived_file`: Path from Step 1 save_orchestrator.py output
   - `session_type`: From Step 1 detection (learn/ask/review)
   - `domain`: From Step 2 classification-expert
   - `subdomain`: Generate slug from main topic (e.g., "qing-dynasty")
   - `isced_path`: From Step 2 classification-expert
   - `agent`: "main"
   - `tags`: Extract key keywords from conversation

3. **Build complete JSON file with full session_metadata**:
...
4. **Write complete JSON to new file** (don't overwrite Step 5 output):
```bash
cat > /tmp/enriched_rems_complete.json << 'EOF'
{
  "session_metadata": { ... complete metadata from step 2 above ... },
  "rems": [ ... from Step 5 enriched Rems ... ],
  "rems_to_update": [ ... from Step 4, empty for learn/ask sessions ... ]
}
EOF
```

5. **Execute post-processor with complete metadata**:
...
**Key points:**
- ✅ Main agent constructs complete session_metadata (only agent has conversation context)
- ✅ workflow_orchestrator.py only enriches Rems with typed_relations (utility role)
- ✅ Output paths auto-generated by Step 5 (no need to call get-next-number.py manually)
- ❌ Never pass incomplete metadata to save_post_processor.py (will cause KeyError)
