...
- prompt: "
  ...
  **⛔ MCQ RULE (if format=multiple-choice)**:
  ...
  - **⚠️ STEM LEAK RULE**: The question stem must NOT contain or reveal the correct answer. Strip the tested element from the stem (e.g., remove articles when testing gender, remove translations when testing meaning). Verify: can a student determine the answer from the stem alone? If yes, redesign.
