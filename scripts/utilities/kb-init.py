...
class InitReport:
    ...
    def display(self, use_color: bool = True):
        ...
        if "github_graph" in self.sections:
            graph = self.sections["github_graph"]
            if graph.get("checked"):
                if graph.get("repo_exists"):
                    status_str = "✅ Exists"
                    if graph.get("created"):
                        status_str = "✅ Created"
                    print(f"  🌐 GitHub Graph Repo: {status_str}")
                else:
                    print(f"  🌐 GitHub Graph Repo: ⚠️  Not configured")
...
def validate_github_graph(
    ...
    """Check and optionally create the GitHub graph repository for GitHub Pages."""
    status = {"checked": False, "repo_exists": False, "created": False, "pages_enabled": False, "url": ""}

    # Detect current repo and derive graph repo name
    try:
        result = subprocess.run(
    ...
    if not match:
        print("  ⚠️  Cannot detect GitHub remote - skipping graph repo check")
        return status

    github_user = match.group(1)
    current_repo = match.group(2)
    graph_repo = f"{current_repo}-graph"
    graph_url = f"https://github.com/{github_user}/{graph_repo}"
    pages_url = f"https://{github_user}.github.io/{graph_repo}/"

    status["checked"] = True
    status["url"] = graph_url

    # Check if gh CLI is available
    gh_check = subprocess.run(["which", "gh"], capture_output=True)
    if gh_check.returncode != 0:
        print("  ⚠️  gh CLI not found - skipping graph repo check")
        return status

    # Check if repo exists
    check_result = subprocess.run(
        ["gh", "repo", "view", f"{github_user}/{graph_repo}"],
        capture_output=True, text=True
    )
    ...
    if check_result.returncode == 0:
        ...
        if verbose:
            print(f"  ✅ Graph repo exists: {graph_url}")
        return status

    # Repo doesn't exist
    print(f"  ⚠️  Graph repo missing: {github_user}/{graph_repo}")

    if dry_run:
        print(f"     Would create: {graph_url}")
        return status

    if not (repair_all or non_interactive or prompt_yes_no(f"Create {graph_repo} on GitHub?", default=True)):
        if report:
            report.add_issue(f"Graph repo not created: {graph_url}")
        return status

    # Create the repo
    create_result = subprocess.run(
        ["gh", "repo", "create", graph_repo, "--public",
         "--description", f"Knowledge Graph Visualizations - Auto-generated by {current_repo}"],
        capture_output=True, text=True
    )

    if create_result.returncode == 0:
        print(f"  ✅ Created: {graph_url}")
        status["repo_exists"] = True
        status["created"] = True
        if report:
            report.add_repair(f"Created GitHub graph repo: {graph_url}")
        print(f"  📊 Graph will be published at: {pages_url}")
        print(f"  💡 Run /graph to generate and deploy visualizations")
    else:
        print(f"  ❌ Failed to create repo: {create_result.stderr.strip()}")
        if report:
            report.add_issue(f"Failed to create graph repo: {create_result.stderr.strip()}")
...
def kb_init(
    ...
    # Phase 7: GitHub Graph Repository Setup
    print("\n🌐 Checking GitHub graph repository...")
    graph_status = validate_github_graph(dry_run, non_interactive, verbose, report, repair_all)
    report.add_section("github_graph", graph_status)
